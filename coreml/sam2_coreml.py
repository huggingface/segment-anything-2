...
class SAM2CoreMLPredictor:
    ...
    def load_models(self):
        self.image_encoder = ct.models.MLModel(
            f"{self.model_dir}/sam2_{self.variant}_image_encoder.mlpackage"
        )
        self.prompt_encoder = ct.models.MLModel(
            f"{self.model_dir}/sam2_{self.variant}_prompt_encoder.mlpackage"
        )
        self.mask_decoder = ct.models.MLModel(
            f"{self.model_dir}/sam2_{self.variant}_mask_decoder.mlpackage"
        )
    ...
    def decode_mask(
        self, image_embedding, sparse_embeddings, dense_embeddings, feats_s0, feats_s1
    ):
    ...
    def preprocess_prompt(self, points, labels):
        ...
        unnorm_coords = self._transforms.transform_coords(
            points, normalize=True, orig_hw=self.orig_hw
        )
    ...
    def predict(self, image_path, points, labels):
        ...
        sparse_embeddings, dense_embeddings = self.encode_prompt(
            input_points, input_labels
        )

        # Decode mask
        low_res_masks = self.decode_mask(
            image_embedding, sparse_embeddings, dense_embeddings, feats_s0, feats_s1
        )
